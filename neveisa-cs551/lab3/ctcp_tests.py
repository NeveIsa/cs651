...
def correct_header_fields():
  ...
  #segment.flags.append("ACK")
  if not segment.has_same_flags(ref_segment):
    print segment.flags
    print ref_segment.flags
  ...
  '''\print    (
    segment.seqno == ref_segment.seqno ,
    (segment.ackno == 0 or segment.ackno == ref_segment.ackno) ,
    segment.length == ref_segment.length ,
    segment.has_same_flags(ref_segment) ,
    segment.window == ref_segment.window ,
...
def bidirectional():
  ...
  if sent_str != test_str_send or recv_str != test_str_recv:
    print sent_str,test_str_send
    print recv_str,test_str_recv
    return False
...
def unreliability(flag):
  ...
  write_to(client, test_str)
  time.sleep(TEST_TIMEOUT)
  __isa_read = read_from(server)
  #print "\n->",test_str,"\n->",__isa_read
  if __isa_read != test_str:
    ___xy = 0
    #print test_str,__isa_read
  return  __isa_read== test_str
...
def segment_truncated():
  ...
  write_to(client, test_str)
  time.sleep(TEST_TIMEOUT)
  __isa_read = read_from(server, num_lines=1)
  if  __isa_read != test_str:
    print test_str,__isa_read
    return False
  ...
  time.sleep(TEST_TIMEOUT)
  __isa_read = read_from(server, num_lines=1)
  if  __isa_read == truncated_str:
    print truncated_str,__isa_read
    return False
...
def connection_teardown():
  ...
  server = start_server(flags=["-d"])
  client = start_client(flags=["-d"])
  ...
  client_msgs =  read_debug_messages_from(client)
  server_msgs =  read_debug_messages_from(server)

  #print client_msgs
  #print server_msgs

  return (
    DEBUG_TEARDOWN in client_msgs and
    DEBUG_TEARDOWN in server_msgs
  )
...
def larger_windows():
  ...
  print last_ackno + 3 * MAX_SEG_DATA_SIZE, largest_seg.seqno, last_ackno + 4 * MAX_SEG_DATA_SIZE
